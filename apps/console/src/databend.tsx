...
export default function DatabendConnector({
  client,
  live,
  agents,
  selectedAgentId,
    ...
    try {
      ...
      if (server) {
        ...
        const created = await client.createMcpServer({
          displayName: form.displayName.trim() || 'Databend 数据源',
          endpointUrl,
      ...
      onNotice?.(`Databend 接入完成：\n${steps.join('\n')}`);
      onChanged?.();
    } catch (err) {
      onNotice?.(`Databend 接入失败：${friendly(err)}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="connector-card">
      <div className="card-title">
        <strong>Databend 数据源（MCP 路径 A）</strong>
        <span className="tag tag-live">{live ? 'Live' : 'Mock'}</span>
      </div>
      <p className="hint">
        前置：已用 <code>pnpm databend:bridge</code> 启动本机桥，并经 TLS 反向代理暴露为
