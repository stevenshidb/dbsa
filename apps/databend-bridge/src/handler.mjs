// 把 MCP tools/call 映射到 Databend SQL；结果统一为 text（Agent9 当前只消费 text/link）。

import { databendQuery, parseDsn } from './databend-http.mjs';
import {
  checkSqlSafety,
  newSessionId,
  sandboxPrefix,
...
export function createDatabendHandler({
  dsn,
  safeMode = true,
  timeoutSecs = 300,
  maxRows = 200,
  ...
  const client = (() => {
    ...
    if (!dsn) throw new Error('缺少 DATABEND_DSN（或使用 --mock 运行自测）');
    const parsed = parseDsn(dsn);
    return {
      run: (sql) =>
        databendQuery(parsed, sql, { timeoutSecs, maxRows }),
    };
  })();
