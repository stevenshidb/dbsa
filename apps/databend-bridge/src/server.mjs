...
export async function startMcpServer({
    ...
    if (method === 'initialize') {
      ...
      return {
        ...
        result: {
          ...
          serverInfo: { name: 'tidbsa-databend-bridge', version: '0.1.0' },
          instructions:
            'Databend 数据源桥。安全模式默认开启：只读查询可直接执行；写操作仅允许命中 sandbox 前缀对象。',
        },
      };
    }
  ...
  const httpServer = createServer(async (req, res) => {
    try {
      ...
      logger.error?.('[databend-bridge] unhandled:', error);
      if (!res.headersSent) {
...
export function randomBearer() {
  return `tb_${randomBytes(18).toString('base64url')}`;
}
